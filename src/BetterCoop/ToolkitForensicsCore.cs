...
public static class ToolkitRngCounter
{
  ...
  public static void Clear()
  {
    ...
    ToolkitRngAnalyzer.Clear();
  }
}

public readonly record struct ToolkitRngEvent(
    ulong Sequence,
    byte Stream,
    byte Method,
    ulong CallIndex,
...
public readonly record struct ToolkitRngStreamSummary(
    ulong Calls,
    ulong RollingHash);

public sealed record ToolkitRngSummary(
    byte OriginOrdinal,
...
public static class ToolkitSeedTag
{
  public const int Bytes = 32;

  public static byte[] Create(Guid sessionId, ulong seed)
  {
    if (sessionId == Guid.Empty)
    {
      return [];
    ...
    Span<byte> normalizedSeed = stackalloc byte[sizeof(ulong)];
    ...
    BinaryPrimitives.WriteUInt64BigEndian(normalizedSeed, seed);
    return HMACSHA256.HashData(key, normalizedSeed);
  }
}

public static class ToolkitRngSummaryCodec
{
  public const int StreamCount = 12;
  public const int PayloadBytes =
      1 + sizeof(uint) + sizeof(uint) + sizeof(ulong)
      + ToolkitSeedTag.Bytes
      + StreamCount * (sizeof(ulong) + sizeof(ulong));

  public static bool TryEncode(
      ToolkitRngSummary value,
      out byte[] payload)
  {
    payload = [];
    if (value.OriginOrdinal > ToolkitLimits.MaxPlayers
        ...
        || value.CheckpointId == 0
        || value.SeedTag.Length != ToolkitSeedTag.Bytes
        || value.Streams.Length != StreamCount)
    {
      return false;
    }
    ...
    BinaryPrimitives.WriteUInt64LittleEndian(
        ...
        value.CheckpointId);
    offset += sizeof(ulong);
    value.SeedTag.CopyTo(payload, offset);
    offset += ToolkitSeedTag.Bytes;
    foreach (ToolkitRngStreamSummary stream in value.Streams)
    {
      BinaryPrimitives.WriteUInt64LittleEndian(
          payload.AsSpan(offset),
          stream.Calls);
      ...
      out ToolkitRngSummary value)
  {
    ...
    byte[] seedTag = payload.Slice(
        offset,
        ToolkitSeedTag.Bytes).ToArray();
    offset += ToolkitSeedTag.Bytes;
    ToolkitRngStreamSummary[] streams =
        new ToolkitRngStreamSummary[StreamCount];
    for (int index = 0; index < streams.Length; index++)
    {
      ...
      ulong rollingHash =
          BinaryPrimitives.ReadUInt64LittleEndian(payload[offset..]);
      offset += sizeof(ulong);
      streams[index] = new(calls, rollingHash);
    }
...
public static class ToolkitRngAnalyzer
{
  public const int RecentEventsPerStream = 64;
  ...
  public static readonly string[] MethodIds =
  [
      "NextBool",
      "NextInt(max)",
      "NextInt(range)",
  ...
  private sealed class StreamState
  {
    ...
    public ulong RollingHash { get; set; } = FnvOffset;
    public ToolkitRngEvent[] Events { get; } =
        new ToolkitRngEvent[RecentEventsPerStream];
    public int NextEvent { get; set; }
    public int EventCount { get; set; }
  }

  private static readonly object Sync = new();
  private static readonly StreamState[] Streams =
      Enumerable.Range(0, ToolkitRngSummaryCodec.StreamCount)
          .Select(_ => new StreamState())
          .ToArray();
  private static ulong _sequence;
  private static ulong _unknownCalls;
  private static ulong _chaoticCalls;

  public static ulong UnknownCalls
  {
    get
    {
      lock (Sync)
      {
        return _unknownCalls;
      }
    }
  }

  public static ulong ChaoticCalls
  {
    get
    {
      lock (Sync)
      {
        return _chaoticCalls;
      }
    }
  }
  ...
  public static void Observe(
      ...
      bool chaotic = false)
  {
    if (!ToolkitRngCounter.IsAvailable)
    {
      return;
    }
    ...
    lock (Sync)
    {
      ...
      if ((uint)stream >= (uint)Streams.Length)
      {
        _unknownCalls++;
        return;
      }

      if ((uint)method >= (uint)MethodIds.Length)
      {
        return;
      }

      StreamState state = Streams[stream];
      ulong callIndex = ++state.Calls;
      ulong sequence = ++_sequence;
      state.RollingHash = Mix(state.RollingHash, (ulong)method);
      state.RollingHash = Mix(state.RollingHash, argumentA);
      state.RollingHash = Mix(state.RollingHash, argumentB);
      state.RollingHash = Mix(state.RollingHash, result);
      state.Events[state.NextEvent] = new(
          sequence,
          (byte)stream,
          (byte)method,
          callIndex,
          ...
          result);
      state.NextEvent =
          (state.NextEvent + 1) % RecentEventsPerStream;
      state.EventCount =
          Math.Min(state.EventCount + 1, RecentEventsPerStream);
    }
  }

  public static ToolkitRngStreamSummary[] Snapshot()
  {
    lock (Sync)
    {
      return Streams.Select(state =>
          new ToolkitRngStreamSummary(
              state.Calls,
              state.RollingHash)).ToArray();
    }
  }

  public static ToolkitRngEvent[] Recent(int maxEvents)
  {
    ArgumentOutOfRangeException.ThrowIfLessThan(maxEvents, 0);
    lock (Sync)
    {
      if (maxEvents == 0)
      {
      ...
      return Streams.SelectMany(state =>
          Enumerable.Range(0, state.EventCount)
              .Select(offset =>
              {
                int index =
                    (state.NextEvent - state.EventCount + offset
                     + RecentEventsPerStream)
                    % RecentEventsPerStream;
  ...
  public static void Clear()
  {
    lock (Sync)
    {
      foreach (StreamState state in Streams)
      {
        state.Calls = 0;
        state.RollingHash = FnvOffset;
        Array.Clear(state.Events);
        state.NextEvent = 0;
        state.EventCount = 0;
      }

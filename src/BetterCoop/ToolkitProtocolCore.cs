...
public static class ToolkitLimits
{
...
{
  public const int MaxRows = ToolkitLimits.MaxPlayers;
...
{
  public const int MaxPeers = ToolkitLimits.MaxPlayers;
  ...
  public ToolkitSendLimiter(
      int maxPeers = MaxPeers,
      double tokensPerSecond = 1,
      int burst = DefaultBurst)
  {
    ArgumentOutOfRangeException.ThrowIfLessThan(maxPeers, 1);
    ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(tokensPerSecond, 0);
  ...
  {
    if (peerOrdinal < 1 || peerOrdinal > _maxPeers || nowTicks < 0)
    {
      return false;
    }
    ...
    if (!_peers.TryGetValue(peerOrdinal, out PeerBudget? budget))
    {
      ...
      budget = new PeerBudget(_burst, nowTicks);
    }
    ...
    double tokens = Math.Min(
        _burst,
        budget.Tokens
            + elapsed / (double)Stopwatch.Frequency * _tokensPerSecond);
    if (tokens < 1)
    {
